...
const poisonOnly = pokémon.filter(p => p.types.length === 1 && p.types[0] === "poison").map(p => p.name);
console.log(poisonOnly);

const firstTypeWhenSecondFlying = pokémon.filter(p => p.types[1] === "flying").map(p => p.types[0]);
console.log(firstTypeWhenSecondFlying);
